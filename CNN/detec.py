...
def __probability_map(predictions, locations, window_dim, x_count, y_count, img_width, img_height, img_dim, count):
    ...
    for i in range(0, n):
        ...
        if (x2 - x1) < min_dim or (y2 - y1) < min_dim:
            continue
        newRegion = (predictions[i] * r_factor)
        x1 = int(newRegion[0] + locations[i, 0])
        y1 = int(newRegion[1] + locations[i, 1])
        x2 = int(newRegion[2] + locations[i, 0])
        y2 = int(newRegion[3] + locations[i, 1])
